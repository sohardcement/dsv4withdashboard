...
#define DS4_TOKEN_HISTORY_WINDOW_DAYS 371
...
typedef struct {
	bool persistent;
	ds4_token_history_day days[DS4_TOKEN_HISTORY_WINDOW_DAYS];
	size_t len;
	uint64_t prompt_tokens;
	uint64_t output_tokens;
	uint64_t requests;
	...
	int64_t peak_day;
	uint64_t current_streak;
	uint64_t longest_streak;
	int64_t first_day;
	int64_t last_day;
