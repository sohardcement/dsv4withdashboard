...
":root{color-scheme:light;--wall:#e9eaed;--surface:rgba(255,255,255,.55);--ink:#1b1e24;--muted:#676c74;--line:rgba(27,30,36,.12);--success:#15803d;--danger:#dc2626;--warning:#b45309;--selected:rgba(27,30,36,.07);--control-height:44px;--motion-fast:180ms;--motion-smooth:240ms;--motion-mode:280ms;--instrument-font:SFMono-Regular,Menlo,Consolas,\"Hiragino Sans GB\",\"PingFang SC\",\"Microsoft YaHei\",\"Arial Unicode MS\",monospace}"
"body{margin:0;min-height:100vh;background:linear-gradient(145deg,#f7f8fa 0%,#edf0f3 55%,#e4e7eb 100%);color:var(--ink);overflow-x:hidden}"
".light-field{position:fixed;z-index:-2;width:1100px;height:780px;left:-180px;top:-260px;pointer-events:none;opacity:.42;background:radial-gradient(ellipse at center,rgba(255,255,255,.92) 0%,rgba(255,255,255,.62) 34%,rgba(255,255,255,0) 70%);animation:signal-drift 18s cubic-bezier(.42,0,.58,1) infinite alternate;will-change:transform}"
".light-shadow{position:fixed;z-index:-2;width:780px;height:620px;right:-150px;bottom:-250px;pointer-events:none;opacity:.5;background:radial-gradient(ellipse at center,rgba(148,153,161,.18),rgba(148,153,161,0) 68%);animation:signal-shadow 26s cubic-bezier(.42,0,.58,1) infinite alternate;will-change:transform}"
...
"*{box-sizing:border-box}:focus-visible{outline:3px solid #2563eb;outline-offset:3px}h1,h2,h3,p,dl,dd{margin:0}a{color:inherit}"
...
"button,input,select{min-height:var(--control-height);font:inherit;color:inherit;border:1px solid var(--line);background:var(--surface);border-radius:2px}button{padding:0 18px;cursor:pointer}button.primary{background:var(--ink);border-color:var(--ink);color:#f3f4f6}button:disabled{cursor:not-allowed;opacity:.45}input{width:9rem;padding:0 10px;font-family:var(--instrument-font)}select{padding:0 30px 0 10px}"
...
".mono{font-family:var(--instrument-font)}.eyebrow{display:block;font-size:11px;letter-spacing:.08em;color:var(--muted)}.caption{font-size:12px;color:var(--muted)}"
".page{width:min(100%,1400px);min-height:100vh;margin:0 auto;padding:20px 32px 48px}.topbar{display:grid;grid-template-columns:1fr auto auto;align-items:center;gap:24px;padding-bottom:14px;border-bottom:1px solid var(--line)}"
".brand{display:flex;align-items:baseline;gap:14px;font:800 24px/1 var(--instrument-font);letter-spacing:-.04em}.brand a{display:inline-flex;align-items:center;min-height:var(--control-height);text-decoration:none}.brand .model{font:500 12px/1.4 var(--instrument-font);color:var(--muted);letter-spacing:0}.status-glyph{color:var(--success);font-size:14px;margin-right:6px}"
".mode-switch{display:flex;gap:8px}.mode-switch button[aria-pressed=true]{background:var(--ink);border-color:var(--ink);color:#f3f4f6}.state{display:flex;align-items:center;gap:10px;font:12px/1.4 var(--instrument-font);color:var(--muted)}.state strong{font-weight:600;color:var(--ink)}.status-dot{width:10px;height:10px;border-radius:50%;background:var(--success)}#dashboard.stale .status-dot{background:var(--warning)}"
...
".monitor-title,.management-layout>h1{font-size:20px;letter-spacing:-.02em;margin:16px 0 2px}.mode-layout>.model{color:var(--muted);font-size:13px;margin-bottom:12px}"
".num-wall{position:relative}#monitorMetrics{display:grid;grid-template-columns:minmax(260px,1.55fr) repeat(5,minmax(92px,1fr));grid-template-areas:\"decode prefill phase cache context queue\";gap:18px 20px;align-items:end;overflow:visible;margin:0;padding:12px 0 18px}"
".vital{position:relative;z-index:1;min-width:0}.vital-decode{grid-area:decode}.vital-prefill{grid-area:prefill}.vital-phase{grid-area:phase}.vital-cache{grid-area:cache}.vital-context{grid-area:context}.vital-queue{grid-area:queue}.vital strong{display:block;font-size:18px;font-weight:700;white-space:nowrap;margin-top:4px}.vital-decode strong{font-size:52px;line-height:1.05;letter-spacing:-.03em}.vital-phase strong{white-space:normal;overflow-wrap:anywhere}.metric-subvalue,.metric-phase-meta{display:block;font-size:11px;color:var(--muted);font-family:var(--instrument-font);overflow-wrap:anywhere}.metric-bar{height:3px;background:var(--line);margin-top:6px}.metric-bar span{display:block;height:100%;width:0;background:var(--ink);transition:width 360ms cubic-bezier(.22,.8,.24,1),background-color var(--motion-fast) ease}.value-changing{animation:value-flash var(--motion-fast) ease}@keyframes value-flash{from{color:var(--muted)}to{color:inherit}}"
...
".monitor-grid{display:grid;grid-template-columns:minmax(0,1fr) 400px;gap:18px 24px;align-items:start;margin-top:16px}.monitor-left{display:grid;gap:18px;align-content:start;min-width:0}.monitor-console{margin-left:0;min-width:0}.monitor-panel{padding:10px 14px;min-width:0}.inspector{border-top:1px solid var(--line)}.inspector h2{font-size:15px;margin-bottom:6px}.inspector dl{display:grid;grid-template-columns:auto minmax(0,1fr) auto minmax(0,1fr);gap:3px 12px;font-size:11px}.inspector dt{color:var(--muted);white-space:nowrap}.inspector dd{font-family:var(--instrument-font);word-break:break-all}"
".section-head{display:flex;align-items:baseline;justify-content:space-between;gap:12px;margin-bottom:10px}.section-head h2,.monitor-panel>h2{font-size:15px}.call-filters{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px 10px;margin-bottom:10px}.filter-field{display:flex;flex-direction:column;gap:4px;min-width:0}.filter-field input,.filter-field select{width:100%}"
".call-table-wrap{overflow-x:auto;overflow-y:auto;max-height:320px}.monitor-table{width:100%;min-width:400px;border-collapse:collapse;font-size:13px;line-height:1.35}.monitor-table thead tr,.monitor-table tbody tr{display:grid;grid-template-columns:minmax(0,1.2fr) minmax(0,1fr) minmax(0,.7fr) auto auto;column-gap:10px;align-items:baseline;padding:5px 10px}.monitor-table thead tr{border-bottom:1px solid var(--line)}.monitor-table thead th:nth-child(3),.monitor-table thead th:nth-child(7){display:none}.monitor-table tbody td:nth-child(1){grid-column:1;grid-row:1}.monitor-table tbody td:nth-child(2){grid-column:2;grid-row:1}.monitor-table tbody td:nth-child(3){grid-column:1/4;grid-row:2}.monitor-table tbody td:nth-child(4){grid-column:3;grid-row:1}.monitor-table tbody td:nth-child(5){grid-column:4;grid-row:1}.monitor-table tbody td:nth-child(6){grid-column:5;grid-row:1}.monitor-table tbody td:nth-child(7){grid-column:1/6;grid-row:3}.monitor-table tbody tr{border-bottom:1px solid var(--line);transition:padding-left var(--motion-fast) ease,background-color var(--motion-fast) ease,box-shadow var(--motion-fast) ease}.monitor-table th{font-size:11px;letter-spacing:.06em;color:var(--muted);text-align:left;white-space:nowrap}.monitor-table td{font-family:var(--instrument-font);white-space:nowrap;padding:1px 0}.monitor-table tbody td:nth-child(3),.monitor-table tbody td:nth-child(7){font-size:11px;color:var(--muted);white-space:normal;word-break:break-all}.monitor-table tbody td:nth-child(1),.monitor-table tbody td:nth-child(2),.monitor-table tbody td:nth-child(4),.monitor-table tbody td:nth-child(5),.monitor-table tbody td:nth-child(6){overflow:hidden;text-overflow:ellipsis}.monitor-table tr[aria-selected=true]{background:var(--selected);box-shadow:inset 2px 0 0 var(--ink);padding-left:14px}.request-select{padding:0 12px;font-family:var(--instrument-font)}"
".result{font-weight:600}.result[data-status=active]{color:var(--success)}.result[data-status=failed]{color:var(--danger)}@media(hover:hover) and (pointer:fine){.mode-switch button:hover,.call-filters input:hover,.call-filters select:hover{transform:translateY(-1px);border-color:rgba(27,30,36,.36);box-shadow:0 6px 16px rgba(20,24,30,.08)}.monitor-table tbody tr:not([aria-selected=true]):hover{box-shadow:inset 2px 0 0 rgba(27,30,36,.32);padding-left:12px}}"
".monitor-host{padding:14px 18px;border:1px solid var(--line);border-radius:2px;background:rgba(255,255,255,.35)}.host-ruler{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.host-ruler>div{min-width:0}.host-ruler strong{display:block;font-size:16px;margin-top:4px}"
".management-grid{display:grid;grid-template-columns:minmax(0,8fr) minmax(360px,4fr);gap:18px 24px;align-items:start;margin-top:14px}.management-wall{display:grid;gap:14px;align-content:start;min-width:0}.management-summary,.management-section{padding:14px 18px;border:1px solid var(--line);border-radius:2px;background:rgba(255,255,255,.35)}"
...
".editor{display:grid;gap:8px}.controls{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.unit-label{font-size:11px;color:var(--muted)}.target-state{font-family:var(--instrument-font)}.impact-review{border:1px solid var(--line);border-radius:2px;padding:12px;margin-top:8px}.impact-review dl{display:grid;grid-template-columns:auto minmax(0,1fr);gap:4px 12px;font-size:12px}.impact-review dd{font-family:var(--instrument-font)}.notice{font-size:12px;min-height:18px;color:var(--muted)}.notice.bad{color:var(--danger)}"
"@media(min-width:981px) and (max-width:1307px){.monitor-grid{grid-template-columns:minmax(0,1fr) minmax(360px,400px)}#monitorMetrics{grid-template-columns:repeat(3,minmax(0,1fr));grid-template-areas:\"decode decode prefill\" \"decode decode phase\" \"cache context queue\"}}"
"@media(max-width:980px){.monitor-grid{grid-template-columns:minmax(0,1fr);grid-template-areas:\"wall\" \"console\" \"host\"}.monitor-left{display:contents}.monitor-left>.num-wall{grid-area:wall}.monitor-left>.monitor-host{grid-area:host}.monitor-console{grid-area:console;margin-left:0}#monitorMetrics{grid-template-columns:repeat(3,minmax(0,1fr));grid-template-areas:\"decode decode prefill\" \"decode decode phase\" \"cache context queue\"}.management-grid{grid-template-columns:minmax(0,1fr)}.management-nav{display:none}.settings-grid{grid-template-columns:1fr 1fr}.setting-block+.setting-block{border-top:none;border-left:1px solid var(--line)}}"
"@media(max-width:760px){#monitorMetrics{grid-template-columns:1fr;grid-template-areas:\"decode\" \"prefill\" \"phase\" \"cache\" \"context\" \"queue\"}.vital-decode strong{font-size:40px}.host-ruler{grid-template-columns:1fr}.summary-ruler{grid-template-columns:1fr}.settings-grid{grid-template-columns:1fr}.setting-block+.setting-block{border-left:none;border-top:1px solid var(--line)}.page{padding:16px 16px 40px}.topbar{grid-template-columns:1fr;gap:10px;align-items:start}}"
...
"<header class=\"topbar\"><div class=\"brand\"><a href=\"#managementSummary\"><span class=\"status-glyph\" aria-hidden=\"true\">◆</span>DS4</a><span id=\"model\" class=\"model\">正在读取状态</span></div>"
"<nav class=\"mode-switch\" aria-label=\"Dashboard 模式\"><button type=\"button\" data-mode-choice=\"management\" aria-pressed=\"false\">管理模式</button><button type=\"button\" data-mode-choice=\"monitor\" aria-pressed=\"true\">监控模式</button></nav>"
"<div id=\"connectionState\" class=\"state\"><span id=\"connectionPulse\" class=\"status-dot\" aria-hidden=\"true\"></span><strong id=\"health\" aria-label=\"健康\">等待中</strong><strong id=\"updatedAt\" aria-label=\"更新时间\">尚未更新</strong></div></header>"
...
"<h1 id=\"monitorTitle\" class=\"monitor-title\">实时运行</h1><p class=\"model\">实时吞吐与最近调用检查器</p>"
"<div class=\"monitor-grid\"><div class=\"monitor-left\"><section id=\"monitorMetrics\" class=\"num-wall\" aria-label=\"实时体征\">"
...
"<div class=\"vital vital-queue\"><span class=\"eyebrow\">队列 / 客户端</span><strong id=\"monitorQueue\" class=\"mono metric-value\" data-motion-direction=\"none\"><span class=\"metric-value-window\"><span class=\"metric-value-layer\">—</span></span></strong></div></section>"
"<section id=\"monitorHost\" class=\"monitor-host host-rail\" aria-labelledby=\"monitorHostTitle\"><div class=\"section-head\"><h2 id=\"monitorHostTitle\">主机资源</h2><p class=\"caption\">系统采样不可用时明确标注。</p></div>"
"<div class=\"host-ruler\"><div><span class=\"eyebrow\">内存压力 / Swap</span><strong id=\"monitorHostPressure\" class=\"mono\">不可用</strong></div><div><span class=\"eyebrow\">物理内存 已用 / 总量 / 可用</span><strong id=\"monitorHostPhysical\" class=\"mono\">不可用</strong></div><div><span class=\"eyebrow\">DS4 RSS</span><strong id=\"monitorHostRss\" class=\"mono\">不可用</strong></div></div></section></div>"
"<aside class=\"glass-column monitor-console\" aria-label=\"调用电报与请求检查器\">"
"<section class=\"monitor-panel\" aria-labelledby=\"monitorCallsTitle\"><div class=\"section-head\"><h2 id=\"monitorCallsTitle\">调用电报</h2><p id=\"monitorCallsActive\" class=\"caption\">当前活动请求：—</p></div>"
"<div class=\"call-filters\"><input id=\"callFilterCaller\" data-call-filter=\"caller\" aria-label=\"按调用方筛选\" placeholder=\"调用方\"><select id=\"callFilterClient\" data-call-filter=\"client\" aria-label=\"按服务筛选\"><option value=\"\">全部服务</option></select><select id=\"callFilterApi\" data-call-filter=\"api\" aria-label=\"按 API 筛选\"><option value=\"\">全部 API</option></select><select id=\"callFilterStatus\" data-call-filter=\"result\" aria-label=\"按结果筛选\"><option value=\"\">全部结果</option><option value=\"active\">进行中</option><option value=\"completed\">完成</option><option value=\"failed\">失败</option><option value=\"cancelled\">已取消</option></select></div>"
...
"<aside id=\"requestInspector\" class=\"monitor-panel inspector\" tabindex=\"-1\" aria-labelledby=\"requestInspectorTitle\"><h2 id=\"requestInspectorTitle\">请求检查器</h2><p class=\"caption\">请选择一条调用记录</p></aside></aside></div></section>"
"</main><script>"
...
static void test_dashboard_page_is_served_as_html(void) {
    ...
    TEST_ASSERT(strstr(out, "data-mirror") == NULL);
    TEST_ASSERT(strstr(out, "max-width:1307px") != NULL);
    TEST_ASSERT(strstr(out, "ResizeObserver") != NULL);
    TEST_ASSERT(strstr(out, "body.dashboard-stale .light-field") != NULL);
    TEST_ASSERT(strstr(out, "body:has(#dashboard.stale)") == NULL);
...
extern int ds4_dist_test_interrupt_route_discard_preserves_workers(void);
...
extern int ds4_test_distributed_failure_invalidates_logits(void);
...
static void test_distributed_result_wait_observes_cancellation(void) {
	...
	TEST_ASSERT(ds4_dist_test_interrupt_route_discard_preserves_workers() == 0);
	...
	TEST_ASSERT(ds4_test_distributed_failure_invalidates_logits() == 0);
