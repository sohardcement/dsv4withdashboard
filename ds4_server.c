...
"<h1 id=\"monitorTitle\" class=\"monitor-title\">实时运行</h1><p class=\"model\">推理生命周期、请求轨迹与运行时检查器</p>"
"<section id=\"monitorMetrics\" class=\"num-wall metrics-deck instrument-panel\" aria-label=\"实时体征\">"
...
"<div class=\"monitor-grid\"><div class=\"monitor-left\"><section id=\"inferenceTimeline\" class=\"timeline-panel instrument-panel\" aria-labelledby=\"timelineTitle\"><div class=\"section-head\"><h2 id=\"timelineTitle\">Inference Timeline</h2><p id=\"timelineSummary\" class=\"caption\">等待实时请求</p></div><div class=\"timeline-track\">"
"<div class=\"timeline-stage\" data-timeline-stage=\"prompt\" data-state=\"idle\"><span class=\"stage-name\">PROMPT</span><div class=\"stage-line\"><i class=\"stage-node\"></i></div><span id=\"timelinePrompt\" class=\"stage-value\">—</span></div>"
"<div class=\"timeline-stage\" data-timeline-stage=\"kv\" data-state=\"idle\"><span class=\"stage-name\">KV LOOKUP</span><div class=\"stage-line\"><i class=\"stage-node\"></i></div><span id=\"timelineKv\" class=\"stage-value\">—</span></div>"
"<div class=\"timeline-stage\" data-timeline-stage=\"prefill\" data-state=\"idle\"><span class=\"stage-name\">PREFILL</span><div class=\"stage-line\"><i class=\"stage-node\"></i></div><span id=\"timelinePrefill\" class=\"stage-value\">—</span></div>"
"<div class=\"timeline-stage\" data-timeline-stage=\"expert\" data-state=\"idle\"><span class=\"stage-name\">EXPERT ROUTING</span><div class=\"stage-line\"><i class=\"stage-node\"></i></div><span id=\"timelineExpert\" class=\"stage-value\">遥测未提供</span></div>"
"<div class=\"timeline-stage\" data-timeline-stage=\"decode\" data-state=\"idle\"><span class=\"stage-name\">DECODE</span><div class=\"stage-line\"><i class=\"stage-node\"></i></div><span id=\"timelineDecode\" class=\"stage-value\">—</span></div>"
"<div class=\"timeline-stage\" data-timeline-stage=\"response\" data-state=\"idle\"><span class=\"stage-name\">RESPONSE</span><div class=\"stage-line\"><i class=\"stage-node\"></i></div><span id=\"timelineResponse\" class=\"stage-value\">—</span></div></div></section>"
"<section class=\"monitor-panel instrument-panel\" aria-labelledby=\"monitorCallsTitle\"><div class=\"section-head\"><h2 id=\"monitorCallsTitle\">Request Trace</h2><p id=\"monitorCallsActive\" class=\"caption\">当前活动请求：—</p></div>"
...
"<section id=\"monitorHost\" class=\"monitor-host host-rail instrument-panel\" aria-labelledby=\"monitorHostTitle\"><div class=\"section-head\"><h2 id=\"monitorHostTitle\">主机资源</h2><p class=\"caption\">系统采样不可用时明确标注。</p></div>"
...
"<aside class=\"glass-column monitor-console\" aria-label=\"请求与运行时检查器\"><aside id=\"requestInspector\" class=\"monitor-panel inspector\" tabindex=\"-1\" aria-labelledby=\"requestInspectorTitle\"><h2 id=\"requestInspectorTitle\">Inspector</h2><div id=\"requestInspectorContent\"><p class=\"caption inspector-empty\">请选择一条调用记录</p></div></aside>"
"<section class=\"runtime-inspector\" aria-labelledby=\"runtimeInspectorTitle\"><h2 id=\"runtimeInspectorTitle\">Runtime</h2>"
"<div class=\"inspector-module\"><div class=\"inspector-module-head\"><h3>KV CACHE</h3><strong id=\"inspectorKvUsage\">—</strong></div><p id=\"inspectorKvMeta\">等待容量数据</p><div class=\"inspector-progress\" aria-hidden=\"true\"><span id=\"inspectorKvBar\"></span></div></div>"
"<div class=\"inspector-module\"><div class=\"inspector-module-head\"><h3>EXPERT ROUTING</h3><strong id=\"inspectorExpertValue\">—</strong></div><p id=\"inspectorExpertMeta\">状态接口未提供 MoE 路由遥测</p><div class=\"expert-signal\" aria-hidden=\"true\"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div></div>"
"<div class=\"inspector-module\"><div class=\"inspector-module-head\"><h3>MEMORY USAGE</h3><strong id=\"inspectorMemoryUsage\">—</strong></div><p id=\"inspectorMemoryMeta\">等待主机采样</p><div class=\"inspector-progress\" aria-hidden=\"true\"><span id=\"inspectorMemoryBar\"></span></div></div></section></aside></div></section>"
"</main><script>"
...
"function paintTimeline(s){const p=s.prefill||{},d=s.decode||{},r=s.request||{},calls=s.calls||{},currentId=s.active===true?activeId(calls.active_request_id):null,record=(calls.records||[]).find(call=>currentId&&String(call.request_id)===currentId),active=s.active===true&&!!record,prefilling=active&&s.phase==='prefill',decoding=active&&s.phase==='decode';text('timelineSummary',active?'请求 #'+currentId+' · '+(record.client||'未标识服务'):'等待实时请求');text('timelinePrompt',active?num(r.prompt_tokens).toLocaleString()+' token':'—');text('timelineKv',active?pct(ratio(r.cached_tokens,r.prompt_tokens))+' hit':'—');text('timelinePrefill',active&&nonnegative(p.elapsed_sec)?Number(p.elapsed_sec).toFixed(1)+'s':'—');text('timelineExpert',decoding?'MoE routing · active':'遥测未提供');text('timelineDecode',decoding&&nonnegative(d.elapsed_sec)?Number(d.elapsed_sec).toFixed(1)+'s':'—');text('timelineResponse',active?(r.stream?'stream':'buffered'):'—');timelineState('prompt',active?'done':'idle');timelineState('kv',active?'done':'idle');timelineState('prefill',prefilling?'active':decoding?'done':'idle');timelineState('expert',decoding?'active':'idle');timelineState('decode',decoding?'active':'idle');timelineState('response',decoding&&r.stream?'active':'idle')}"
"function paintRuntimeInspector(s){const k=s.kv_cache||{},h=s.host||{},ku=k.enabled?ratio(k.used_bytes,k.budget_bytes):null,mu=h.available?ratio(h.memory_used_bytes,h.memory_total_bytes):null;text('inspectorKvUsage',k.enabled?pct(ku):'OFF');text('inspectorKvMeta',k.enabled?bytes(k.used_bytes)+' / '+bytes(k.budget_bytes)+' · '+num(k.entries).toLocaleString()+' entries':'磁盘 KV 未配置');$('inspectorKvBar').style.width=Math.max(0,Math.min(100,(ku||0)*100))+'%';text('inspectorExpertValue',s.active&&s.phase==='decode'?'ACTIVE':'IDLE');text('inspectorExpertMeta',s.active&&s.phase==='decode'?'MoE 正在参与解码；状态接口未提供逐专家遥测':'状态接口未提供 MoE 路由遥测');text('inspectorMemoryUsage',h.available?pct(mu):'—');text('inspectorMemoryMeta',h.available?bytes(h.memory_used_bytes)+' / '+bytes(h.memory_total_bytes)+' · RSS '+bytes(h.process_rss_bytes):'等待主机采样');$('inspectorMemoryBar').style.width=Math.max(0,Math.min(100,(mu||0)*100))+'%'}"
...
"function renderSnapshot(s){const m=s.model||{},k=s.kv_cache||{},x=s.context||{},d=s.decode||{},mtp=s.mtp||{};dash.classList.toggle('is-active',s.active===true);text('model',(m.name||'未知模型')+' / '+(m.backend||'未知后端')+' / '+num(m.context_length).toLocaleString()+' token 上下文');text('health',s.active?'运行中':'就绪');text('topTokens',s.active&&s.phase==='decode'&&finite(d.avg_tps)?Number(d.avg_tps).toFixed(1):'—');text('topMtp',finite(mtp.acceptance_rate)?pct(mtp.acceptance_rate):'—');text('topUptime',duration(s.uptime_sec));paintManagement(s);paintMonitor(s);paintTimeline(s);paintRuntimeInspector(s);"
...
static void test_dashboard_page_is_served_as_html(void) {
    ...
    TEST_ASSERT(strstr(out, "<title>DwarfStar Inference Console</title>") != NULL);
    ...
    TEST_ASSERT(strstr(out, "metrics-deck instrument-panel") != NULL);
    ...
    TEST_ASSERT(strstr(out, "data-theme-choice=\"system\"") != NULL);
    TEST_ASSERT(strstr(out, "data-theme-choice=\"dark\"") != NULL);
...
static void test_status_json_reports_idle_metrics_shape(void) {
    ...
    if (sv[0] >= 0 && sv[1] >= 0) {
        ...
        TEST_ASSERT(strstr(out, "\"uptime_sec\":") != NULL);
