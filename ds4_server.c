...
"</main><script>"
"const $=id=>document.getElementById(id),dash=$('dashboard');let lastSnapshot=null,online=false,adminLocal=true,kvEnabled=false,kvState='idle',kvReview=null,kvTrigger=null,pollGeneration=0,contextLocal=true,contextBusy=false,kvTargetTouched=false,contextTargetTouched=false;"
...
"function paint(s){lastSnapshot=s;online=true;dash.classList.remove('stale');const m=s.model||{},k=s.kv_cache||{},x=s.context||{};kvEnabled=k.enabled===true;text('updatedAt',new Date().toLocaleTimeString('zh-CN',{hour12:false}));text('model',(m.name||'未知模型')+' / '+(m.backend||'未知后端')+' / '+num(m.context_length).toLocaleString()+' token 上下文');text('health',s.active?'运行中':'就绪');paintManagement(s);"
