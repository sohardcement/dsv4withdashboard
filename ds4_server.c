...
".page{max-width:1240px;margin:auto;padding:24px 32px 64px}.topbar{display:grid;grid-template-columns:auto auto 1fr;align-items:center;gap:24px;padding-bottom:18px;border-bottom:2px solid var(--ink)}h1,h2,p{margin:0}.brand{display:inline-flex;align-items:center;gap:9px;color:inherit;text-decoration:none;font:800 24px/1 var(--instrument-font);letter-spacing:-.04em}.status-glyph{color:var(--accent);font-size:14px}.model{margin:14px 0 0;color:var(--muted)}.mode-switch{display:flex;gap:0}.mode-switch button[aria-pressed=true]{background:var(--ink);color:var(--surface)}.state{display:flex;gap:18px;align-items:center;justify-content:flex-end}.status-dot{width:9px;height:9px;border-radius:50%;background:var(--success)}.eyebrow{color:var(--muted);font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase}.state strong{font-family:var(--instrument-font);font-weight:500}.phase{color:var(--accent)}"
...
"table{width:100%;border-collapse:collapse;font-size:13px}th,td{text-align:left;padding:10px 12px 10px 0;border-top:1px solid var(--line);vertical-align:top}th{color:var(--muted);font-weight:500;width:16%}td{font-family:var(--instrument-font);overflow-wrap:anywhere}.stale .phase{color:var(--danger)}.stale .status-dot{background:var(--danger)}"
"@media(max-width:760px){.page{padding:16px 16px 44px}.topbar{grid-template-columns:1fr}.mode-switch button{flex:1}.state{justify-content:flex-start;gap:12px}.hero{grid-template-columns:1fr}.hero>div{padding:32px 0}.hero>div+div{border-left:0;border-top:1px solid var(--line);padding-left:0}.strip,.process,.capacity-grid,.capacity-stats{grid-template-columns:1fr}.datum+.datum{border-left:0;border-top:1px solid var(--line);padding-left:0}.capacity-grid{gap:26px}.editor{border-left:0;border-top:1px solid var(--line);padding:26px 0 0}.hero-value{font-size:72px}.controls>*{flex:1 1 auto}input{font-size:16px;width:min(11rem,60vw)}.section-head{display:block}.section-head .caption{margin-top:8px}}"
...
"</style></head><body><main class=\"page\" id=\"dashboard\" data-mode=\"management\"><header class=\"topbar\"><a class=\"brand\" href=\"#managementSummary\"><span class=\"status-glyph\" aria-hidden=\"true\">◆</span>DS4</a><nav class=\"mode-switch\" aria-label=\"Dashboard 模式\"><button type=\"button\" data-mode-choice=\"management\" aria-pressed=\"true\">管理模式</button><button type=\"button\" data-mode-choice=\"monitor\" aria-pressed=\"false\">监控模式</button></nav><div id=\"connectionState\" class=\"state\"><span class=\"status-dot\" aria-hidden=\"true\"></span><strong id=\"health\" aria-label=\"健康\">等待中</strong><strong id=\"updatedAt\" aria-label=\"更新时间\">—</strong></div></header><p id=\"model\" class=\"model\">正在读取状态</p><div id=\"managementLayout\" class=\"mode-layout management-layout\" aria-hidden=\"false\"><section id=\"managementSummary\" class=\"paper-conclusion\" aria-label=\"运行结论\"><div class=\"conclusion-health\"><span class=\"eyebrow\">当前运行结论</span><strong id=\"paperHealthConclusion\" data-bind=\"health\">等待中</strong><p id=\"phase\" class=\"caption\">连接中</p></div><div><span class=\"eyebrow\">上下文摘要</span><strong id=\"paperContextSummary\" data-bind=\"contextCurrent\">—</strong><p data-bind=\"contextRemaining\" class=\"caption\">—</p></div><div><span class=\"eyebrow\">KV 缓存摘要</span><strong id=\"paperCacheSummary\" data-bind=\"requestHitRate\">—</strong><p data-bind=\"kvUtilization\" class=\"caption\">—</p></div></section>"
...
"function paint(s){lastSnapshot=s;online=true;dash.classList.remove('stale');const m=s.model||{},r=s.request||{},p=s.prefill||{},d=s.decode||{},t=s.totals||{},c=t.cache||{},k=s.kv_cache||{};text('updatedAt',new Date().toLocaleTimeString('zh-CN',{hour12:false}));text('model',(m.name||'Unknown model')+' / '+(m.backend||'unknown backend')+' / '+num(m.context_length).toLocaleString()+' token context');text('phase',s.phase||'idle');text('health',s.active?'active':'ready');"
