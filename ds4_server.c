...
".page{max-width:1240px;margin:auto;padding:24px 32px 64px}.topbar{display:grid;grid-template-columns:auto auto 1fr;align-items:center;gap:24px;padding-bottom:18px;border-bottom:2px solid var(--ink)}h1,h2,h3,p{margin:0}.brand{font:800 24px/1 var(--instrument-font);letter-spacing:-.04em}.brand a{display:inline-flex;min-height:44px;align-items:center;gap:9px;color:inherit;text-decoration:none}.status-glyph{color:var(--accent);font-size:14px}.model{margin-top:8px;color:var(--muted)}.mode-switch{display:flex;gap:0}.mode-switch button[aria-pressed=true]{background:var(--ink);color:var(--surface)}.state{display:flex;gap:18px;align-items:center;justify-content:flex-end}.status-dot{width:9px;height:9px;border-radius:50%;background:var(--success)}.eyebrow{color:var(--muted);font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase}.unit-label{align-self:center;color:var(--muted);font-size:12px;font-weight:700}.state strong{font-family:var(--instrument-font);font-weight:500}.caption{color:var(--muted)}.mono{font-family:var(--instrument-font);font-variant-numeric:tabular-nums}"
...
"function validateSnapshot(s){if(!s||typeof s!=='object'||Array.isArray(s))throw new Error('invalid snapshot');for(const key of ['model','kv_cache','context','calls'])if(!s[key]||typeof s[key]!=='object'||Array.isArray(s[key]))throw new Error('invalid '+key);if(!Array.isArray(s.calls.records)||!s.calls.records.every(record=>record&&typeof record==='object'&&!Array.isArray(record)))throw new Error('invalid calls.records')}"
...
"function renderSnapshot(s){const m=s.model||{},k=s.kv_cache||{},x=s.context||{};text('model',(m.name||'未知模型')+' / '+(m.backend||'未知后端')+' / '+num(m.context_length).toLocaleString()+' token 上下文');text('health',s.active?'运行中':'就绪');paintManagement(s);paintMonitor(s);"
...
"function paint(s){validateSnapshot(s);const previous=lastSnapshot;try{renderSnapshot(s)}catch(error){if(previous)renderSnapshot(previous);throw error}lastSnapshot=s;lastUpdatedAt=Date.now();online=true;kvEnabled=s.kv_cache.enabled===true;dash.classList.remove('stale');text('updatedAt',freshnessLabel(false));contextControls();setKvState(kvState)}"
