...
".management-section{padding:30px 0;border-bottom:1px solid var(--ink)}.section-head{display:flex;justify-content:space-between;align-items:baseline;gap:20px;margin-bottom:20px}.recent-calls{table-layout:fixed}.recent-calls th,.recent-calls td{width:25%}.host-ruler{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border-top:1px solid var(--line)}.host-ruler>div{padding:16px 16px 0 0}.host-ruler>div+div{padding-left:16px;border-left:1px solid var(--line)}.host-ruler strong{display:block;margin-top:7px;overflow-wrap:anywhere}.detail-transition{margin-top:30px;padding-top:16px;border-top:1px dashed var(--muted)}.detail-transition .section-head{margin-bottom:14px}"
...
"@media(max-width:900px){.management-grid{grid-template-columns:1fr;gap:0;margin-top:20px}.management-nav{display:none}}"
"@media(max-width:760px){.page{padding:16px 16px 44px}.topbar{grid-template-columns:1fr}.mode-switch button{flex:1}.state{justify-content:flex-start;gap:12px}.summary-ruler,.settings-grid,.capacity-stats,.host-ruler{grid-template-columns:1fr}.summary-cell+.summary-cell,.host-ruler>div+div{padding-left:0;border-left:0;border-top:1px solid var(--line)}.settings-grid{gap:28px}.controls>*{flex:1 1 auto}input{font-size:16px;width:min(11rem,60vw)}.section-head,.setting-head{display:block}.section-head .caption,.effect{display:block;margin-top:8px}}"
"@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important}}"
".call-filters{display:flex;gap:8px;flex-wrap:wrap}.call-filters input,.call-filters select{min-width:0}.call-table-wrap{overflow-x:auto}.call-table-wrap table{min-width:640px}.mode-layout[hidden]{display:none!important}"
...
"<section id=\"managementRecent\" class=\"management-section\"><div class=\"section-head\"><h2>最近调用</h2><p id=\"callsActive\" class=\"caption\">当前活动请求：—</p></div><table class=\"recent-calls\"><thead><tr><th>请求</th><th>服务</th><th>API</th><th>结果</th></tr></thead><tbody id=\"managementRecentCalls\"></tbody></table></section>"
"<section id=\"managementHost\" class=\"management-section\"><div class=\"section-head\"><h2>主机资源</h2><p class=\"caption\">系统采样不可用时明确标注。</p></div><div class=\"host-ruler\"><div><span class=\"eyebrow\">内存压力 / Swap</span><strong id=\"managementHostPressure\" class=\"mono\">不可用</strong></div><div><span class=\"eyebrow\">物理内存 已用 / 总量 / 可用</span><strong id=\"managementHostPhysical\" class=\"mono\">不可用</strong></div><div><span class=\"eyebrow\">DS4 RSS</span><strong id=\"managementHostRss\" class=\"mono\">不可用</strong></div></div></section>"
"<section class=\"detail-transition\" id=\"callsSection\" aria-labelledby=\"callsDetailTitle\"><div class=\"section-head\"><h2 id=\"callsDetailTitle\">完整调用明细（过渡）</h2><p class=\"caption\">详细检查器将在监控模式中继续演进。</p></div><div class=\"call-filters\"><input id=\"callFilterCaller\" data-call-filter=\"caller\" aria-label=\"按调用方筛选\" placeholder=\"调用方\"><select id=\"callFilterClient\" data-call-filter=\"client\" aria-label=\"按服务筛选\"><option value=\"\">全部服务</option></select><select id=\"callFilterApi\" data-call-filter=\"api\" aria-label=\"按 API 筛选\"><option value=\"\">全部 API</option></select><select id=\"callFilterStatus\" data-call-filter=\"result\" aria-label=\"按结果筛选\"><option value=\"\">全部结果</option><option value=\"active\">进行中</option><option value=\"completed\">完成</option><option value=\"failed\">失败</option></select></div><div class=\"call-table-wrap\"><table><thead><tr><th>请求</th><th>服务</th><th>调用方</th><th>API</th><th>结果</th><th>错误</th></tr></thead><tbody id=\"callsRecords\"></tbody></table></div><h3>服务 / 调用方汇总</h3><div class=\"call-table-wrap\"><table><thead><tr><th>服务</th><th>调用方</th><th>调用</th><th>失败</th><th>提示 token</th></tr></thead><tbody id=\"callsCallers\"></tbody></table></div></section></div></div></div>"
"<section id=\"monitorLayout\" class=\"mode-layout monitor-layout\" hidden aria-hidden=\"true\" aria-label=\"监控模式\"></section>"
"</main><script>"
"const $=id=>document.getElementById(id),dash=$('dashboard');let lastSnapshot=null,online=false,adminLocal=true,kvState='idle',kvReview=null,kvTrigger=null,pollGeneration=0,contextLocal=true,contextBusy=false,kvTargetTouched=false,contextTargetTouched=false;"
"const num=n=>Number.isFinite(Number(n))?Number(n):0,ratio=(n,d)=>num(d)>0?num(n)/num(d):null,pct=v=>v==null?'—':(v*100).toFixed(1)+'%';"
...
"function setKvState(state){kvState=state;dash.dataset.kvState=state;$('kvReview').hidden=state!=='review';const k=(lastSnapshot&&lastSnapshot.kv_cache)||{},locked=['checking','review','applying','saving'].includes(state)||!online||!adminLocal||!k.enabled;for(const id of ['kvBudgetInput','kvBudgetUnit','kvApplyNow','kvSaveRestart'])$(id).disabled=locked}"
...
"function paintCalls(calls){calls=calls||{};const records=(calls.records||[]).slice(0,200),caller=$('callFilterCaller').value.toLowerCase(),client=$('callFilterClient').value,api=$('callFilterApi').value,status=$('callFilterStatus').value;replaceOptions('callFilterClient',[...new Set(records.map(x=>x.client||''))].filter(Boolean),'全部服务');replaceOptions('callFilterApi',[...new Set(records.map(x=>x.api||''))].filter(Boolean),'全部 API');const shown=records.filter(x=>(!caller||String(x.caller||'').toLowerCase().includes(caller))&&(!client||x.client===client)&&(!api||x.api===api)&&(!status||x.status===status)).map(x=>({...x,status:callStatus(x.status)}));callRows('callsRecords',shown,['request_id','client','caller','api','status','error']);callRows('callsCallers',calls.callers||[],['client','caller','calls','failed','prompt_tokens']);text('callsActive','当前活动请求：'+(calls.active_request_id||'—'))}"
...
"function paintHost(h){if(!h||!h.available){text('managementHostPhysical','不可用');text('managementHostPressure','不可用');text('managementHostRss','不可用');return}const physical=[h.memory_used_bytes,h.memory_total_bytes,h.memory_available_bytes],swap=[h.swap_used_bytes,h.swap_total_bytes];text('managementHostPhysical',physical.every(v=>Number.isFinite(Number(v)))?physical.map(bytes).join(' / '):'不可用');text('managementHostPressure',pressure(h.memory_pressure)!=='不可用'&&swap.every(v=>Number.isFinite(Number(v)))?pressure(h.memory_pressure)+' / '+swap.map(bytes).join(' / '):'不可用');text('managementHostRss',Number.isFinite(Number(h.process_rss_bytes))?bytes(h.process_rss_bytes):'不可用')}"
...
"function finishKvSuccess(message){kvReview=null;setKvState('success');setKvMessage(message,false);if(kvTrigger)kvTrigger.focus()}"
"function finishKvError(error){kvReview=null;setKvState('error');setKvMessage(kvErrorMessage(error),true);if(kvTrigger)kvTrigger.focus()}"
"function reviewFingerprint(r){return JSON.stringify([num(r.old_budget_bytes),num(r.new_budget_bytes),num(r.before_bytes),num(r.before_entries),num(r.after_bytes),num(r.after_entries)])}"
...
"function checkedRuntime(body){const r=body&&body.runtime||{};if(!r.ok||r.revision==null)throw kvLocalError('运行时预检未返回有效修订版本。');return r}"
"async function checkKvImpact(){if(['checking','review','applying','saving'].includes(kvState))return;kvTrigger=$('kvApplyNow');let mb;try{mb=budgetMB()}catch(e){finishKvError(e);return}setKvState('checking');setKvMessage('正在检查清理压力…',false);try{showKvReview(checkedRuntime(await admin('dry-run',mb)),mb)}catch(e){finishKvError(e)}}"
"async function confirmKvApply(){if(kvState!=='review'||!kvReview)return;const reviewed=kvReview;setKvState('applying');setKvMessage('正在应用已审阅的运行时上限…',false);try{let applied;try{applied=await admin('apply',reviewed.mb,String(reviewed.runtime.revision))}catch(e){if(e.code!=='kv_state_changed')throw e;setKvMessage('KV 状态已变化，正在重新检查影响…',false);const refreshed=checkedRuntime(await admin('dry-run',reviewed.mb)),fingerprint=reviewFingerprint(refreshed);if(fingerprint!==reviewed.fingerprint){showKvReview(refreshed,reviewed.mb);return}try{applied=await admin('apply',reviewed.mb,String(refreshed.revision))}catch(retry){if(retry.code==='kv_state_changed')throw kvLocalError('KV 状态持续变化，请等待活动缓存操作结束后重试。','kv_state_changed');throw retry}}if(!(applied.runtime&&applied.runtime.ok&&applied.runtime.applied))throw kvLocalError('服务器未确认已应用运行时上限。');finishKvSuccess(runtimeMessage(applied))}catch(e){finishKvError(e)}}"
"function cancelKvApply(){if(kvState!=='review')return;kvReview=null;setKvState('idle');setKvMessage('已取消应用；运行时容量未改变。',false);if(kvTrigger)kvTrigger.focus()}"
"async function persistKvBudget(){if(['checking','review','applying','saving'].includes(kvState))return;kvTrigger=$('kvSaveRestart');let mb;try{mb=budgetMB()}catch(e){finishKvError(e);return}setKvState('saving');setKvMessage('正在保存下次启动上限…',false);try{const saved=await admin('persist',mb),p=saved.persistent||{};if(!p.committed)throw kvLocalError('服务器未提交下次启动设置。');if(!p.ok||!p.durable){kvReview=null;setKvState('error');setKvMessage(persistentMessage(saved),true);if(kvTrigger)kvTrigger.focus();return}finishKvSuccess(persistentMessage(saved))}catch(e){finishKvError(e)}}"
...
static void test_dashboard_page_is_served_as_html(void) {
    ...
    TEST_ASSERT(strstr(out, "服务 / 调用方汇总") != NULL);
    TEST_ASSERT(strstr(out, "['request_id','client','caller','api','status','error']") != NULL);
    ...
    TEST_ASSERT(strstr(out, "before_entries)+' → '+num(r.after_entries)") != NULL);
