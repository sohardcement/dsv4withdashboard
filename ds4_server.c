...
"function chineseStatic(){$('currentTitle').textContent='当前请求 / KV token 命中';document.querySelectorAll('.hero .eyebrow').forEach((e,i)=>e.textContent=['当前请求 / KV token 命中','解码 / 平均'][i]);document.querySelectorAll('.hero .caption').forEach(e=>e.textContent='—');document.querySelectorAll('.strip .eyebrow').forEach((e,i)=>e.textContent=['预填充','解码进度','当前会话','请求'][i]);document.querySelectorAll('.process .eyebrow').forEach((e,i)=>e.textContent=['token 命中率','请求命中率','结果'][i]);document.querySelectorAll('#kvCapacity .capacity-stats .eyebrow').forEach((e,i)=>e.textContent=['已用','容量上限','条目 / 利用率'][i]);document.querySelectorAll('.section h2').forEach((e,i)=>e.textContent=['进程记录','磁盘 KV 容量','上下文窗口','主机资源','调用记录','请求详情'][i]);document.querySelectorAll('.section-head .caption').forEach((e,i)=>{if(i<4)e.textContent=['计数器会在此服务器进程重启时清零。','索引磁盘缓存；上方实时上下文以 token 计。','数值以 token 计，不代表 RAM。','系统采样不可用时明确标注。'][i]});$('kvForm').querySelector('label').textContent='容量上限';$('kvBudgetUnit').setAttribute('aria-label','容量单位');$('budgetHelp').textContent='最小 256 MB。立即应用会修改运行时状态；保存会写入下次启动上限。';$('kvApplyNow').textContent='立即应用';$('kvSaveRestart').textContent='保存供重启后使用'}chineseStatic();"
