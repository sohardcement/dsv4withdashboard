...
"</style></head><body><main class=\"page\" id=\"dashboard\" data-mode=\"management\"><header class=\"topbar\"><div class=\"brand\"><a href=\"#managementSummary\"><span class=\"status-glyph\" aria-hidden=\"true\">◆</span>DS4</a></div><nav class=\"mode-switch\" aria-label=\"Dashboard 模式\"><button type=\"button\" data-mode-choice=\"management\" aria-pressed=\"true\">管理模式</button><button type=\"button\" data-mode-choice=\"monitor\" aria-pressed=\"false\">监控模式</button></nav><div id=\"connectionState\" class=\"state\"><span class=\"status-dot\" aria-hidden=\"true\"></span><strong id=\"health\" aria-label=\"健康\">等待中</strong><strong id=\"updatedAt\" aria-label=\"更新时间\">尚未更新</strong></div></header><div id=\"managementLayout\" class=\"mode-layout management-layout\" aria-hidden=\"false\"><div class=\"management-grid\"><aside class=\"management-nav\"><nav aria-label=\"管理章节\"><a href=\"#managementSummary\">运行概览</a><a href=\"#kvCapacity\">KV 容量</a><a href=\"#contextCapacity\">上下文窗口</a><a href=\"#managementRecent\">最近调用</a><a href=\"#managementHost\">主机资源</a></nav></aside><div class=\"management-main\">"
...
"</main><script>"
"const $=id=>document.getElementById(id),dash=$('dashboard');let lastSnapshot=null,lastUpdatedAt=0,online=false,adminLocal=true,kvEnabled=false,kvState='idle',kvReview=null,kvTrigger=null,pollGeneration=0,contextLocal=true,contextBusy=false,kvTargetTouched=false,contextTargetTouched=false,selectedRequestId=null;"
...
"function paint(s){lastSnapshot=s;lastUpdatedAt=Date.now();online=true;dash.classList.remove('stale');const m=s.model||{},k=s.kv_cache||{},x=s.context||{};kvEnabled=k.enabled===true;text('updatedAt',freshnessLabel(false));text('model',(m.name||'未知模型')+' / '+(m.backend||'未知后端')+' / '+num(m.context_length).toLocaleString()+' token 上下文');text('health',s.active?'运行中':'就绪');paintManagement(s);paintMonitor(s);"
...
"async function tick(){const generation=++pollGeneration;try{const response=await fetch('/ds4/status',{cache:'no-store'});if(!response.ok)throw new Error('status '+response.status);const snapshot=await response.json();if(generation===pollGeneration)paint(snapshot)}catch(e){if(generation===pollGeneration){online=false;dash.classList.add('stale');text('health',lastSnapshot?'数据已过期':'不可用');text('updatedAt',freshnessLabel(true));setKvState(kvState);contextControls()}}finally{if(generation===pollGeneration)setTimeout(tick,1000)}}$('kvApplyNow').addEventListener('click',checkKvImpact);$('kvSaveRestart').addEventListener('click',persistKvBudget);$('kvConfirmApply').addEventListener('click',confirmKvApply);$('kvCancelApply').addEventListener('click',cancelKvApply);$('contextSaveRestart').addEventListener('click',saveContext);tick();"
"</script></body></html>\n";
