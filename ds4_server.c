...
"<!doctype html><html lang=\"zh-CN\" data-theme=\"paper\"><head><meta charset=\"utf-8\">"
"<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
"<title>DS4 运行报告</title><style>"
":root{color-scheme:light;--paper:#f3f0e7;--surface:#f8f5ed;--ink:#171a1d;--muted:#706d65;--line:#c4bfb4;--accent:#df4932;--success:#28734b;--danger:#a52a1c;--selected:#e7dfcf;--control-height:44px;--motion-fast:180ms;--motion-smooth:360ms;--instrument-font:SFMono-Regular,Menlo,Consolas,\"Hiragino Sans GB\",\"PingFang SC\",\"Microsoft YaHei\",\"Arial Unicode MS\",monospace}"
":root[data-theme=terminal]{color-scheme:dark;--paper:#0d1117;--surface:#151b23;--ink:#ecf2f8;--muted:#93a4b8;--line:#314154;--accent:#58a6ff;--success:#56d364;--danger:#ff7b72;--selected:#1d3550}"
":root[data-theme=calm]{color-scheme:light;--paper:#e8f1f0;--surface:#f5fbfa;--ink:#12333b;--muted:#5a777a;--line:#b7cfcc;--accent:#0e7490;--success:#28734b;--danger:#b9382f;--selected:#d3e9e5}"
"*{box-sizing:border-box}:focus-visible{outline:3px solid #2563eb;outline-offset:3px}body,button,input,select,.mode-switch button,.theme-switch button,.status-dot,.metric-bar span{transition:background-color var(--motion-fast) ease,color var(--motion-fast) ease,border-color var(--motion-fast) ease,box-shadow var(--motion-fast) ease}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.5 -apple-system,BlinkMacSystemFont,\"Hiragino Sans GB\",\"PingFang SC\",\"Microsoft YaHei\",\"Arial Unicode MS\",\"Helvetica Neue\",Arial,sans-serif}"
...
".page{max-width:1240px;margin:auto;padding:24px 32px 64px}.topbar{display:grid;grid-template-columns:auto auto auto 1fr;align-items:center;gap:24px;padding-bottom:18px;border-bottom:2px solid var(--ink)}h1,h2,h3,p{margin:0}.brand{font:800 24px/1 var(--instrument-font);letter-spacing:-.04em}.brand a{display:inline-flex;min-height:44px;align-items:center;gap:9px;color:inherit;text-decoration:none}.status-glyph{color:var(--accent);font-size:14px}.model{margin-top:8px;color:var(--muted)}.mode-switch,.theme-switch{display:flex;gap:0}.mode-switch button[aria-pressed=true]{background:var(--ink);color:var(--surface)}.theme-switch button{padding-inline:12px;color:var(--muted);border-color:var(--line)}.theme-switch button[aria-pressed=true]{background:var(--accent);border-color:var(--accent);color:var(--surface)}.state{display:flex;gap:18px;align-items:center;justify-content:flex-end}.status-dot{width:9px;height:9px;border-radius:50%;background:var(--success)}.eyebrow{color:var(--muted);font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase}.unit-label{align-self:center;color:var(--muted);font-size:12px;font-weight:700}.state strong{font-family:var(--instrument-font);font-weight:500}.caption{color:var(--muted)}.mono{font-family:var(--instrument-font);font-variant-numeric:tabular-nums}"
".management-grid{display:grid;grid-template-columns:180px minmax(0,1fr);gap:40px;margin-top:28px}.management-nav{border-right:1px solid var(--line);padding-right:22px}.management-nav nav{position:sticky;top:24px;display:grid;gap:2px}.management-nav a{min-height:46px;display:flex;align-items:center;border-top:1px solid var(--line);color:var(--muted);text-decoration:none}.management-nav a:first-child{border-top:2px solid var(--ink);color:var(--ink)}.management-main{min-width:0}.management-summary{padding-bottom:30px;border-bottom:2px solid var(--ink)}.management-summary h1{font:400 clamp(42px,6vw,72px)/.95 Georgia,\"Times New Roman\",serif;letter-spacing:-.04em}.summary-ruler{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));margin-top:28px;border-top:1px solid var(--ink);border-bottom:1px solid var(--ink)}.summary-cell{min-width:0;padding:18px 18px 20px 0}.summary-cell+.summary-cell{padding-left:18px;border-left:1px solid var(--line)}.summary-cell strong{display:block;margin:8px 0 5px;font:400 clamp(22px,3vw,34px)/1.05 Georgia,serif;overflow-wrap:normal;word-break:keep-all}.summary-cell:nth-child(2) strong{font-size:clamp(20px,2.65vw,30px);white-space:nowrap}.summary-cell:first-child strong{color:var(--accent)}"
...
".mode-layout[hidden]{display:none!important}:root[data-theme=glass]{color-scheme:light;--paper:#eef1f6;--surface:rgba(255,255,255,.55);--ink:#1d1d1f;--muted:#6e6e73;--line:rgba(0,0,0,.08);--accent:#0071e3;--success:#248a3d;--danger:#d70015;--selected:rgba(0,113,227,.12);--glass-bg:rgba(255,255,255,.55);--glass-border:rgba(255,255,255,.65);--glass-shadow:0 12px 32px rgba(31,38,135,.14),inset 0 1px 0 rgba(255,255,255,.7)}:root[data-theme=glass] body{background:radial-gradient(60vw 60vw at 12% 8%,rgba(165,200,255,.8),transparent 60%),radial-gradient(50vw 50vw at 88% 12%,rgba(255,196,214,.75),transparent 60%),radial-gradient(55vw 55vw at 82% 88%,rgba(195,240,224,.75),transparent 60%),radial-gradient(45vw 45vw at 18% 92%,rgba(230,217,255,.75),transparent 60%),linear-gradient(180deg,#f5f7fb,#e8ecf4);background-attachment:fixed;min-height:100vh}:root[data-theme=glass] body::before{content:'';position:fixed;inset:0;z-index:3;pointer-events:none;background:radial-gradient(420px 420px at var(--mx,50%) var(--my,18%),rgba(255,255,255,.55),transparent 70%);mix-blend-mode:soft-light}:root[data-theme=glass] .topbar{background:var(--glass-bg);backdrop-filter:blur(24px) saturate(1.8);-webkit-backdrop-filter:blur(24px) saturate(1.8);border:1px solid var(--glass-border);border-radius:20px;padding:10px 16px;box-shadow:var(--glass-shadow)}:root[data-theme=glass] .management-nav,:root[data-theme=glass] .management-summary,:root[data-theme=glass] .setting-block,:root[data-theme=glass] .management-section,:root[data-theme=glass] .monitor-panel,:root[data-theme=glass] .monitor-host{background:var(--glass-bg);backdrop-filter:blur(24px) saturate(1.8);-webkit-backdrop-filter:blur(24px) saturate(1.8);border:1px solid var(--glass-border);border-radius:20px;box-shadow:var(--glass-shadow);padding:20px 22px}:root[data-theme=glass] .management-grid{gap:28px}:root[data-theme=glass] .summary-ruler{border-top-color:var(--line);border-bottom-color:var(--line)}:root[data-theme=glass] .management-nav a:first-child{border-top-color:var(--line)}:root[data-theme=glass] .settings-grid,:root[data-theme=glass] .monitor-table-wrap{border-bottom-color:var(--line)}:root[data-theme=glass] .management-summary h1,:root[data-theme=glass] .monitor-title,:root[data-theme=glass] .setting-head h2,:root[data-theme=glass] .management-section h2,:root[data-theme=glass] .summary-cell strong,:root[data-theme=glass] .monitor-metrics strong{font-family:-apple-system,BlinkMacSystemFont,sans-serif;font-weight:700}:root[data-theme=glass] button,:root[data-theme=glass] input,:root[data-theme=glass] select{border-radius:12px;border-color:rgba(0,0,0,.14);background:rgba(255,255,255,.5)}:root[data-theme=glass] button.primary{background:linear-gradient(180deg,#2b9bff,#0071e3);color:#fff;border-color:transparent;box-shadow:0 6px 16px rgba(0,113,227,.35)}:root[data-theme=glass] button:hover:not(:disabled){box-shadow:0 4px 14px rgba(0,0,0,.14)}:root[data-theme=glass] .request-select{background:transparent;border-color:transparent;border-radius:8px}:root[data-theme=glass] .request-select:hover{background:rgba(255,255,255,.5);border-color:transparent}:root[data-theme=glass] .mode-switch,:root[data-theme=glass] .theme-switch{background:rgba(255,255,255,.4);border:1px solid var(--glass-border);border-radius:14px;padding:3px}:root[data-theme=glass] .mode-switch button,:root[data-theme=glass] .theme-switch button{background:transparent;border-color:transparent;min-height:36px;border-radius:11px;padding-inline:14px}:root[data-theme=glass] .mode-switch button[aria-pressed=true]{background:#1d1d1f;color:#fff}:root[data-theme=glass] .theme-switch button{color:var(--muted)}:root[data-theme=glass] .theme-switch button[aria-pressed=true]{background:linear-gradient(180deg,#2b9bff,#0071e3);border-color:transparent;color:#fff}:root[data-theme=glass] .monitor-metrics>div{background:rgba(255,255,255,.45);border:1px solid var(--glass-border);border-radius:16px;padding:12px 14px;backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);box-shadow:0 6px 18px rgba(31,38,135,.08)}:root[data-theme=glass] .monitor-metrics>div.metric-primary{background:rgba(255,255,255,.7)}:root[data-theme=glass] .progress,:root[data-theme=glass] .metric-bar{height:6px;border-radius:99px;background:rgba(0,0,0,.08)}:root[data-theme=glass] .progress span,:root[data-theme=glass] .metric-bar span{border-radius:99px;background:linear-gradient(90deg,#5ac8fa,#0071e3);box-shadow:0 0 8px rgba(0,113,227,.5)}:root[data-theme=glass] .status-dot{box-shadow:0 0 8px var(--success)}:root[data-theme=glass] .impact-review{border-radius:14px;padding:14px 16px 16px;background:rgba(255,255,255,.4)}"
...
"function paint(s){validateSnapshot(s);const previous=lastSnapshot;try{renderSnapshot(s)}catch(error){if(previous)renderSnapshot(previous);throw error}lastSnapshot=s;lastUpdatedAt=Date.now();online=true;kvEnabled=s.kv_cache.enabled===true;currentKvBudgetBytes=kvEnabled?s.kv_cache.budget_bytes:null;dash.classList.remove('stale');text('updatedAt',freshnessLabel(false));updateKvTargetState();contextControls();setKvState(kvState)}"
