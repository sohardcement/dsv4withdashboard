...
"</style></head><body><main class=\"page\" id=\"dashboard\" data-mode=\"management\" data-theme=\"paper\"><header class=\"topbar\"><div class=\"brand\"><a href=\"#managementSummary\"><span class=\"status-glyph\" aria-hidden=\"true\">◆</span>DS4</a></div><nav class=\"mode-switch\" aria-label=\"Dashboard 模式\"><button type=\"button\" data-mode-choice=\"management\" aria-pressed=\"true\">管理模式</button><button type=\"button\" data-mode-choice=\"monitor\" aria-pressed=\"false\">监控模式</button></nav><div class=\"theme-switch\" role=\"group\" aria-label=\"Dashboard 主题\"><button type=\"button\" data-theme-choice=\"paper\" aria-pressed=\"true\">暖纸面</button><button type=\"button\" data-theme-choice=\"terminal\" aria-pressed=\"false\">深色终端</button><button type=\"button\" data-theme-choice=\"calm\" aria-pressed=\"false\">冷静蓝绿</button></div><div id=\"connectionState\" class=\"state\"><span class=\"status-dot\" aria-hidden=\"true\"></span><strong id=\"health\" aria-label=\"健康\">等待中</strong><strong id=\"updatedAt\" aria-label=\"更新时间\">尚未更新</strong></div></header><div id=\"managementLayout\" class=\"mode-layout management-layout\" aria-hidden=\"false\"><div class=\"management-grid\"><aside class=\"management-nav\"><nav aria-label=\"管理章节\"><a href=\"#managementSummary\">运行概览</a><a href=\"#kvCapacity\">KV 容量</a><a href=\"#contextCapacity\">上下文窗口</a><a href=\"#managementRecent\">最近调用</a><a href=\"#managementHost\">主机资源</a></nav></aside><div class=\"management-main\">"
...
"document.querySelectorAll('[data-theme-choice]').forEach(b=>b.addEventListener('click',()=>setTheme(b.dataset.themeChoice)));try{setTheme(localStorage.getItem('ds4-dashboard-theme'))}catch(e){setTheme('paper')}"
"const STATUS_TIMEOUT_MS=2000;/* Bound a status request without overlapping client polls. */async function tick(){const generation=++pollGeneration,controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),STATUS_TIMEOUT_MS);try{const response=await fetch('/ds4/status',{cache:'no-store',signal:controller.signal});if(!response.ok)throw new Error('status '+response.status);const snapshot=await response.json();if(generation===pollGeneration)paint(snapshot)}catch(e){if(generation===pollGeneration){ds4MetricMotionReset();online=false;dash.classList.add('stale');text('health',lastSnapshot?'数据已过期':'不可用');text('updatedAt',freshnessLabel(true));setKvState(kvState);contextControls()}}finally{clearTimeout(timeout);if(generation===pollGeneration)setTimeout(tick,1000)}}$('kvApplyNow').addEventListener('click',checkKvImpact);$('kvSaveRestart').addEventListener('click',persistKvBudget);$('kvConfirmApply').addEventListener('click',confirmKvApply);$('kvCancelApply').addEventListener('click',cancelKvApply);$('contextSaveRestart').addEventListener('click',saveContext);setInterval(()=>text('updatedAt',freshnessLabel(!online)),1000);setKvState(kvState);contextControls();tick();"
