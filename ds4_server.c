...
"function setTheme(value){const theme=['paper','terminal','calm'].includes(value)?value:'paper';dash.dataset.theme=theme;for(const name of ['paper','terminal','calm']){const root=$(name+'Layout'),active=name===theme;root.hidden=!active;root.setAttribute('aria-hidden',String(!active))}try{localStorage.setItem('ds4-dashboard-theme',theme)}catch(e){}document.querySelectorAll('[data-theme-choice]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.themeChoice===theme)))}try{setTheme(localStorage.getItem('ds4-dashboard-theme'))}catch(e){setTheme('paper')}document.querySelectorAll('[data-theme-choice]').forEach(b=>b.addEventListener('click',()=>setTheme(b.dataset.themeChoice)));['callFilterCaller','callFilterApi','callFilterStatus'].forEach(id=>$(id).addEventListener('input',()=>paintCalls((lastSnapshot||{}).calls)));"
