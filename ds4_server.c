...
"<section id=\"managementRecent\" class=\"management-section\"><div class=\"section-head\"><h2>最近调用</h2><p id=\"callsActive\" class=\"caption\">当前活动请求：—</p></div><div id=\"managementRecentCalls\" aria-live=\"polite\"></div></section>"
...
"function paintRecentCalls(calls){const root=$('managementRecentCalls');root.replaceChildren();for(const call of ((calls&&calls.records)||[]).slice(0,3)){const row=document.createElement('div');row.className='recent-call';row.dataset.callId=String(call.request_id||'');for(const value of [call.request_id,call.client,call.api,callStatus(call.status)]){const cell=document.createElement('span');cell.textContent=value==null||value===''?'—':String(value);row.append(cell)}root.append(row)}}"
...
"if(!k.enabled){text('kvUsed','已禁用');text('kvBudget','已禁用');text('kvEntries','—');text('kvUtilization','未配置磁盘 KV 缓存。');$('kvBar').style.width='0%'}else{const ku=ratio(k.used_bytes,k.budget_bytes);text('kvUsed',bytes(k.used_bytes));text('kvBudget',bytes(k.budget_bytes));text('kvEntries',num(k.entries).toLocaleString());text('kvUtilization',pct(ku)+' 已使用');$('kvBar').style.width=Math.max(0,Math.min(100,(ku||0)*100))+'%';if(!$('kvForm').contains(document.activeElement)){$('kvBudgetInput').value=(num(k.budget_bytes)/1073741824).toFixed(2).replace(/\\.?0+$/,'');$('kvBudgetUnit').value='GB'}}text('contextCurrent',num(x.current_tokens).toLocaleString()+' / '+num(x.limit_tokens).toLocaleString());text('contextRemaining',num(x.remaining).toLocaleString());text('contextUtilization',pct(x.utilization));if(!$('contextForm').contains(document.activeElement))$('contextNextInput').value=num(x.next_limit_tokens)||num(x.limit_tokens)||'';contextControls();paintCalls(s.calls);controls(k)}"
...
static void test_dashboard_page_is_served_as_html(void) {
    ...
    TEST_ASSERT(strstr(out, "admin('apply',mb,revision)") != NULL);
	...
	TEST_ASSERT(strstr(out, "adminBusy") != NULL);
	TEST_ASSERT(strstr(out, "adminGeneration") != NULL);
	TEST_ASSERT(strstr(out, "if(adminBusy)return") != NULL);
