...
"const $=id=>document.getElementById(id),dash=$('dashboard');let lastSnapshot=null,online=false,adminLocal=true;"
...
"function controls(kv){const disabled=!online||!adminLocal||!kv.enabled;$('kvApplyNow').disabled=disabled;$('kvSaveRestart').disabled=disabled;$('kvBudgetInput').disabled=!online||!adminLocal||!kv.enabled;$('kvBudgetUnit').disabled=!online||!adminLocal||!kv.enabled}"
...
"async function admin(mode,mb){const response=await fetch('/ds4/admin/kv-cache',{method:'POST',headers:{'Content-Type':'application/json','X-DS4-Admin':'1'},body:JSON.stringify({mode,budget_mb:mb})});let body;try{body=await response.json()}catch(e){throw new Error('The server returned an unreadable admin response.')}if(response.status===403){adminLocal=false;if(lastSnapshot)controls(lastSnapshot.kv_cache||{});throw new Error('Capacity controls are available only from localhost.')}if(!body.ok&&!(mode==='persist'&&body.persistent&&body.persistent.committed)){throw new Error((body.error&&body.error.message)||'The capacity operation failed.')}return body}"
"function runtimeMessage(body){const r=body.runtime||{},evicted=Math.max(0,num(r.before_entries)-num(r.after_entries));return 'Runtime: '+bytes(r.before_bytes)+' → '+bytes(r.after_bytes)+', '+evicted+' entr'+(evicted===1?'y':'ies')+' evicted; limit '+bytes(r.old_budget_bytes)+' → '+bytes(r.new_budget_bytes)+'.'}"
...
"async function applyNow(){try{const mb=budgetMB();text('adminNotice','Checking eviction pressure…');$('adminNotice').className='notice';const dry=await admin('dry-run',mb),r=dry.runtime||{};if(!r.ok)throw new Error('The runtime dry-run did not succeed.');if(r.eviction_required&&!confirm('Current disk KV usage is '+bytes(r.before_bytes)+'. The new limit is '+bytes(r.new_budget_bytes)+'. Applying it may evict indexed entries. Continue?')){text('adminNotice','Apply cancelled; runtime capacity was not changed.');return}const applied=await admin('apply',mb);if(!(applied.runtime&&applied.runtime.ok&&applied.runtime.applied))throw new Error('The server did not confirm that the runtime limit was applied.');text('adminNotice',runtimeMessage(applied));await tick()}catch(e){$('adminNotice').className='notice bad';text('adminNotice',e.message)}}"
"async function saveRestart(){try{const mb=budgetMB();text('adminNotice','Saving next-start limit…');$('adminNotice').className='notice';const saved=await admin('persist',mb),p=saved.persistent||{};if(!p.committed)throw new Error('The server did not commit the restart setting.');text('adminNotice',persistentMessage(saved));if(!p.ok||!p.durable)$('adminNotice').className='notice bad'}catch(e){$('adminNotice').className='notice bad';text('adminNotice',e.message)}}"
"async function tick(){try{const response=await fetch('/ds4/status',{cache:'no-store'});if(!response.ok)throw new Error('status '+response.status);paint(await response.json())}catch(e){online=false;dash.classList.add('stale');text('phase','offline');text('health',lastSnapshot?'stale':'unavailable');controls((lastSnapshot&&lastSnapshot.kv_cache)||{})}}$('kvApplyNow').addEventListener('click',applyNow);$('kvSaveRestart').addEventListener('click',saveRestart);tick();setInterval(tick,1000);"
"</script></body></html>\n";
...
static void test_dashboard_page_is_served_as_html(void) {
    ...
    TEST_ASSERT(strstr(out, "admin('apply',mb)") != NULL);
