...
"function paintMonitor(s){const p=s.prefill||{},d=s.decode||{},r=s.request||{},x=s.context||{},calls=s.calls||{},snapshotActive=s.active===true,currentId=snapshotActive?activeId(calls.active_request_id):null,active=(calls.records||[]).find(call=>currentId&&String(call.request_id)===currentId),current=!!active,phase=({decode:'解码中',prefill:'预填充中',idle:'空闲'})[s.phase]||'未知阶段';text('monitorPhase',phase+' · '+(snapshotActive?'运行中':'就绪')+' · '+(current&&active.client||'—'));text('monitorPrefill',current&&['prefill','decode'].includes(s.phase)&&finite(p.avg_tps)?Number(p.avg_tps).toFixed(1)+' t/s · '+num(p.current).toLocaleString()+' / '+num(p.total).toLocaleString():'不可用');text('monitorDecode',current&&s.phase==='decode'&&finite(d.avg_tps)?Number(d.avg_tps).toFixed(1)+' t/s · '+num(d.generated).toLocaleString()+' / '+num(d.max_tokens).toLocaleString():'不可用');text('monitorCacheHit',current&&ratio(r.cached_tokens,r.prompt_tokens)!=null?pct(ratio(r.cached_tokens,r.prompt_tokens)):'不可用');text('monitorContext',finite(x.utilization)?pct(Number(x.utilization)):'不可用');text('monitorQueue',finite(s.queue_depth)&&finite(s.clients)?num(s.queue_depth).toLocaleString()+' / '+num(s.clients).toLocaleString():'不可用');paintCalls(calls,r,snapshotActive)}"
