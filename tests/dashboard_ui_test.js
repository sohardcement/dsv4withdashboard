async page => {
  ...
  const desktop=await page.evaluate(()=>({w:document.documentElement.scrollWidth,v:innerWidth,text:document.body.innerText})); assert(desktop.w<=desktop.v&&desktop.text.includes('纸面运行报告')&&desktop.text.includes('上下文窗口'),'desktop layout or Chinese labels missing');
  await page.setViewportSize({width:390,height:844}); await wait(50); const mobile=await page.evaluate(()=>({w:document.documentElement.scrollWidth,v:innerWidth})); assert(mobile.w<=mobile.v,'mobile dashboard overflows horizontally'); await page.setViewportSize({width:1440,height:900});
  ...
  await cfg({reset:true}); await page.reload(); await wait(100); await page.locator('#callFilterCaller').fill('direct'); await page.locator('#callFilterApi').selectOption('responses'); await page.locator('#callFilterStatus').selectOption('active'); assert((await page.locator('#callsRecords').innerText()).includes('direct'),'call filters did not leave direct record'); await page.locator('#callFilterCaller').fill('恶意'); await page.locator('#callFilterApi').selectOption('chat'); await page.locator('#callFilterStatus').selectOption('failed'); assert(await page.locator('#callsRecords').locator('img,script').count()===0&&(await page.locator('#callsRecords').innerText()).includes('<script>坏</script>'),'malicious calls text was parsed as markup');
