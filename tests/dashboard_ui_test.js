async page => {
  ...
  const managementRecentText=await page.locator('#managementRecentCalls').innerText(); assert(await page.locator('#managementRecentCalls [data-call-id]').count()===3&&managementRecentText.includes('hanako-agent')&&managementRecentText.includes('responses'),'management recent calls are incomplete');
